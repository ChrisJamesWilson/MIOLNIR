...
ID = t[0]
tt = np.stack((Teff,logg,Z), axis=1)
...
for j in range(len(ID)):
	...
	print('logg: ' + str(np.log10(logg_new)))
	print('Z: ' + str(Z_new))
...
#				newstars = dist[(abs(theta[stars[0]]-theta) >
	...
	if radii >= 0.5:
		...
		diffs = abs(int_spectra-t_rl[:,1])/t_rl[:,1]
		...
		np.savetxt('./Stellar_Spectra/' + file_spectra + '.txt', ttt)
...
for i in range(len(t[0])):
	...
	logg = t[2][i]
	Z = t[3][i]
	file_name = ret.set_spectra_name(Teff, logg, Z)
	tt = np.loadtxt('./Stellar_Spectra/' + file_name + '.txt')
	if i == 0:
